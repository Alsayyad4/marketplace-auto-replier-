...
- **Demo videos sent/resent multiple times in one chat (v0.12.2)** → root cause: `chatAlreadyHasOurVideo()` only looked at `[dir="auto"]` duration badges + real `<video>` elements, but FB renders a sent clip as a thumbnail + play button with the duration in a **plain overlay span (not `[dir="auto"]`) and no `<video>` until you press play** — so detection returned false, `injectVideo`'s preview-detach confirm was sometimes unsure (`okCount=0`), `landed` came out false, a fail was recorded, and the 20-min backoff **retried → another copy**. Fix: `chatAlreadyHasOurVideo()` now scans **every leaf span/div in message rows** for a badge-sized (`≤90px`) element whose own text is exactly `mm:ss`, still gated to our side + a real `[role="row"]` (excludes the pinned listing card); and the post-send `landed` check **polls the chat for ~9s** before declaring failure so a slow-rendering upload isn't misread and retried.
